  ...
  for (const attestation of attestations) {
    ...
    for (const index of attestingIndices) {
      ...
      if (totalWeight > 0) {
        ...
        const effectiveBalance = epochCtx.effectiveBalances.get(index);
        if (!effectiveBalance) throw Error(`No effectiveBalance for index ${index}`); // Should never happen
        totalBalancesWithWeight += effectiveBalance * totalWeight;
      }
    }
  }
